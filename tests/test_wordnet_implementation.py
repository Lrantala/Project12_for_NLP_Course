...
def test_return_whole_synset_list_from_word():
    ...
    computer_synsets = wni.return_synset_list_for_word(word, whole_synset=True)
...
def test_return_name_synset_list_from_word():
    word = "computer"
    test_computer_synsets = ["computer.n.01", "calculator.n.01"]
    computer_synsets = wni.return_synset_list_for_word(word, whole_synset=False)
    assert test_computer_synsets == computer_synsets


def test_return_hyponym_list_for_synset():
    computer_synset = wni.return_synset_list_for_word("computer")
    computer_synset_1 = computer_synset[0]
    computer_hyponyms = wni.return_hyponym_or_hypernym_list_for_synset(computer_synset_1,
                                                                       whole_synset=False,
                                                                       hypo_or_hyper=0)
